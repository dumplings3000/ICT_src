...
class Node1:
    def __init__(self):
        ...
        # self.lower_light_blue = np.array([100, 43, 46])
        # self.upper_light_blue = np.array([124, 100, 100])
        self.lower_light_blue = np.array([90, 120, 50])
        self.upper_light_blue = np.array([130, 230, 255])
        ...
        # self.init_observe_pose_joint = [-1.5746906672569194, 0.21244009383951182, -1.645535146468826, -0.13358555843794687, -1.5712132446635485, -0.003277340392985478]
        self.init_grasp_pose = [-0.45866827392578126, 0.43749786376953126, 0.19912411499023438, -3.14, 0, -1.57]
        self.middle_place_pose = [-0.1550082244873047, 0.5790004272460938, 0.20202963256835937, -3.14, 0, -3.14]
    ...
    def stage_switch(self, msg):
        ...
        while(self.state != "finish" and self.block_num < 5):
            ...
            # x axis part
            place_pose[0] += (index[1] - 1) * 0.11
            # y axis part
            place_pose[1] += (1 - index[0]) * 0.13

            pre_place_pose = copy.deepcopy(place_pose)
            pre_place_pose[2] += 0.055
